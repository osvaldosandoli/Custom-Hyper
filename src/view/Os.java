...
public class Os extends JDialog {
	...
	public Os() {
		...
		btnNewButton.setBounds(734, 395, 131, 139);
		...
		desktopPane.setBounds(374, 109, 491, 164);
		getContentPane().add(desktopPane);

		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 11, 471, 142);
		...
		btnAdicionar.setBounds(624, 441, 111, 82);
		...
		lblNewLabel_11 = new JLabel("Tipo de Servico");
		lblNewLabel_11.setFont(new Font("Microsoft JhengHei", Font.ITALIC, 11));
		lblNewLabel_11.setBounds(204, 63, 92, 14);
		...
		lblNewLabel_10.setBounds(428, 298, 196, 20);
		getContentPane().add(lblNewLabel_10);
		
		txtRetirada = new JTextField();
		txtRetirada.setBounds(613, 298, 170, 20);
	...
	private void pesquisarOs() {
		...
		try {
			...
			if(rs.next()){
				...
				if (rs.getString(2).equals("orcamento")) {
				...
				txtServicos.setText(rs.getString(6));
				txtTecnico.setText(rs.getString(7));
				txtValor.setText(rs.getString(8));
				txtRetirada.setText(rs.getString(9));
				txtId.setText(rs.getString(10)); 
				txtData.setText(rs.getString(11));
				
				
			}
			
			else {
					JOptionPane.showMessageDialog(null, "Numero de OS nao existe", "Atencao", JOptionPane.ERROR_MESSAGE);
	...
	private void adicionarOs(){
		...
		if (cboStatus.getSelectedItem().equals("")) {
			JOptionPane.showMessageDialog(null, "Preencha o campo Status", "Atencao", JOptionPane.ERROR_MESSAGE);
			cboStatus.requestFocus();
	} else if (txtServicos.getText().isEmpty()) {
		JOptionPane.showMessageDialog(null, "Preencha o campo Servicos", "Atencao", JOptionPane.ERROR_MESSAGE);
		txtServicos.requestFocus();
	} else if (txtModelo.getText().isEmpty()) {
		JOptionPane.showMessageDialog(null, "Preencha o Modelo do Tenis","Atencao", JOptionPane.ERROR_MESSAGE);
		txtModelo.requestFocus();
	} else if (txtMarca.getText().isEmpty()) {
		JOptionPane.showMessageDialog(null, "Preencha a Marca do Tenis, ", "Atencao", JOptionPane.ERROR_MESSAGE);
		txtMarca.requestFocus();
	} else if (txtTecnico.getText().isEmpty()) {
		JOptionPane.showMessageDialog(null, "Informe o personalizador da Ordem de Servico", "Atencao", JOptionPane.ERROR_MESSAGE);
		txtTecnico.requestFocus();
	} else if (txtValor.getText().isEmpty()) {
		JOptionPane.showMessageDialog(null,"Preencha o Valor do Servico","Atencao", JOptionPane.ERROR_MESSAGE);
	...
	private void excluirOs() {
		...
		int confirma = JOptionPane.showConfirmDialog(null, "Confirma a exclus�o dessa Ordem de Servico ?", "Atencao!", JOptionPane.YES_NO_OPTION);
		...
		if (confirma == JOptionPane.YES_OPTION) {
			...
			try {
				...
				if(excluir == 1) {
					JOptionPane.showMessageDialog(null, "OS excluida com sucesso!!!", "Atencao", JOptionPane.WARNING_MESSAGE);
				}
	...
	// --------------------------------------------------------------------------------------------------------
	  ...
	  private void imprimirOs() {
		  	...
		  	Connection con = dao.conectar();
		  int confirma = JOptionPane.showConfirmDialog(null, "Confirma a impress�o desta OS?", "Aten��o", JOptionPane.YES_NO_OPTION);
	        if (confirma == JOptionPane.YES_OPTION) {
	            try {
	...
	// --------------------------------------------------------------------------------------------------------
		...
		private void limparBotao() {
			int confirma = JOptionPane.showConfirmDialog(null, "Deseja limpar a tela de OS", "Atencao", JOptionPane.YES_NO_OPTION);
			if(confirma == JOptionPane.YES_OPTION) { 
			txtId.setText(null);
